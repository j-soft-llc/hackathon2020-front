    ...
    <div class="vue-poll">
        ...
        <div class="ans-cnt">
            <div
              ...
              :key="index" :class="{ ans: true, [a.custom_class]: (a.custom_class) }">
...
export default {
  ...
  computed: {
    totalVotes() {
      ...
      this.answers.filter((a) => {
        ...
        if (!isNaN(a.votes) && a.votes > 0) totalVotes += parseInt(a.votes, 10);
      });
      return totalVotes;
    },
    totalVotesFormatted() {
    ...
    calcAnswers() {
      ...
      return this.answers.filter((a) => {
        // eslint-disable-next-line no-restricted-globals
        if (!isNaN(a.votes) && a.votes > 0) a.percent = `${Math.round((parseInt(a.votes, 10) / this.totalVotes) * 100)}%`;
        else a.percent = '0%';
  ...
  methods: {
    handleMultiple() {
      ...
      const obj = { arSelected, totalVotes: this.totalVotes };
